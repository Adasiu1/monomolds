...
export function CartPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    repriceCart(items, "inpost_locker", giftItems).then((result) => {
      if (cancelled) return;
      if (result.ok) {
  ...
  return <div className="site-container cart-page">
    ...
    {items.length === 0 ? <div className="cart-empty"><h2>Koszyk jest pusty</h2><p>Wybierz produkty do zamówienia.</p><LinkButton href="/sklep">Przejdź do form</LinkButton></div> : <div className="cart-layout">
      <section aria-labelledby="cart-items-heading">
        ...
        <ul className="cart-items">
          {displayItems.map((item) => <li key={item.merchandiseId} className="cart-item">
            ...
            <strong className="cart-item-price">{item.lineTotalGrosze === null ? "…" : <>{formatPrice(item.lineTotalGrosze)} <small>brutto</small>{item.unitNetPriceGrosze === null ? null : <small>{formatPrice(item.unitNetPriceGrosze * item.quantity)} netto</small>}</>}</strong>
      ...
      {quote ? <aside className={`cart-summary${quoteMatchesCart ? "" : " cart-summary--updating"}`} aria-label="Podsumowanie koszyka" aria-busy={!quoteMatchesCart}><h2>Podsumowanie</h2><p>{paidItemsSummary(totalItems, quote.giftPromotion.earnedQuantity)}</p><OrderBenefits physicalItemCount={quoteMatchesCart ? quote.physicalItemCount : undefined} /><strong>{quoteMatchesCart ? <>{formatPrice(quote.totalGrosze)} <small>brutto</small></> : "…"}</strong>{quote.giftPromotion.selectedItems.reduce((sum, item) => sum + item.quantity, 0) < quote.giftPromotion.earnedQuantity ? <><Button disabled>Najpierw wybierz gratis</Button><p className="cart-summary-hint">Wybierz wszystkie należne gratisy poniżej produktów.</p></> : <LinkButton href="/zamowienie">Przejdź do wysyłki</LinkButton>}</aside> : <aside className="cart-summary cart-summary--updating" aria-label="Podsumowanie koszyka" aria-busy="true"><h2>Podsumowanie</h2><p>{paidItemsSummary(totalItems, 0)}</p><strong>…</strong></aside>}
    </div>}
  </div>;
}
